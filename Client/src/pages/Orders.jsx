...
//     try {
//       await axios.put(`http://localhost:8080/api/order/update-order/${orderId}`, { status: newStatus });
//       fetchOrders();
//     } catch (err) {
...
//       name: "Payment",
//       cell: (row) => {
//         const paymentStatus = row.payment?.status || 'unpaid';
//         return (
...
//                 <div>
//                   <h3 className="text-lg font-bold">Order #{selectedOrder._id.slice(-6).toUpperCase()}</h3>
...
//                     <p><span className="font-medium">Address:</span> {selectedOrder.shippingAddress?.address || 'N/A'}</p>
//                     <p><span className="font-medium">City:</span> {selectedOrder.shippingAddress?.city || 'N/A'}, {selectedOrder.shippingAddress?.state || 'N/A'} {selectedOrder.shippingAddress?.zipCode || 'N/A'}</p>
//                   </div>
//                 </div>
...
//                       <div className="flex justify-between">
//                         <span>Discount:</span>
//                         <span className="text-red-500">-₹{selectedOrder.totalPrice - selectedOrder.totalPriceAfterDiscount}</span>
//                       </div>
//                     )}
//                     <div className="flex justify-between font-bold border-t pt-2">
//                       <span>Total:</span>
//                       <span>₹{selectedOrder.totalPriceAfterDiscount || selectedOrder.totalPrice}</span>
//                     </div>
                    
//                     <h4 className="font-bold mt-4 mb-2 text-gray-700">Payment Information</h4>
//                     <p><span className="font-medium">Status:</span> <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
...
//                         'bg-red-100 text-red-800'}`}>
//                       {(selectedOrder.payment?.status || 'unpaid').toUpperCase()}
//                     </span></p>
//                     {selectedOrder.payment?.method && (
//                       <p><span className="font-medium">Method:</span> {selectedOrder.payment.method.toUpperCase()}</p>
//                     )}
//                   </div>
//                 </div>
//               </div>
...
//                                 <img className="h-10 w-10 rounded object-cover" src={item.product?.images[0]?.url || 'https://via.placeholder.com/40'} alt={item.product?.name} />
//                               </div>
//                               <div className="ml-4">
//                                 <div className="text-sm font-medium text-gray-900">{item.product?.name || 'Product'}</div>
//                                 <div className="text-sm text-gray-500">{item.color?.title || 'N/A'}</div>
//                               </div>
//                             </div>
//                           </td>
//                           <td className="px-4 py-3 text-sm text-gray-500">₹{item.price || 0}</td>
//                           <td className="px-4 py-3 text-sm text-gray-500">{item.quantity || 0}</td>
//                           <td className="px-4 py-3 text-sm text-gray-500">₹{(item.price || 0) * (item.quantity || 0)}</td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
const Orders = () => {
  ...
  const fetchPaymentDetails = async (orderId) => {
    try {
      const res = await axios.get(`http://localhost:8080/api/payment/order/${orderId}`);
    ...
    },
    {
      ...
      cell: (row) => (
        <button 
          ...
          className="px-3 py-1 bg-green-50 text-green-600 text-xs rounded hover:bg-green-100"
        >
          {row.payment?.status === 'paid' ? 'View Payment' : 'Add Payment'}
        </button>
      ),
      width: "120px",
    },
                      ...
                      <>
                        <p><span className="font-medium">Status:</span> <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                          ${paymentDetails.status === 'paid' ? 'bg-green-100 text-green-800' : 
                            ...
                            'bg-red-100 text-red-800'}`}>
                          {(paymentDetails.status || 'unpaid').toUpperCase()}
                        </span></p>
                        <p><span className="font-medium">Method:</span> {paymentDetails.method?.toUpperCase() || 'N/A'}</p>
                        <p><span className="font-medium">Amount:</span> ₹{paymentDetails.amount || selectedOrder.totalPriceAfterDiscount || selectedOrder.totalPrice}</p>
                        <p><span className="font-medium">Transaction ID:</span> {paymentDetails.transactionId || 'N/A'}</p>
                        <p><span className="font-medium">Date:</span> {paymentDetails.createdAt ? new Date(paymentDetails.createdAt).toLocaleString() : 'N/A'}</p>
                      </>
                    ) : (
